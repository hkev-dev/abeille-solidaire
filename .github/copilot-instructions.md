...
* **Flower Cycle Completion and Upgrade:** When a user receives 4 `direct` donations in their current flower:
    ...
    * If the user has a parent in the matrix, a `Donation` record with `donation_type` = 'matrix_propagation' is created, where the donor is the user who completed the cycle and the recipient is their parent. The amount is the donation amount of the user's *next* flower.
